...
std::unordered_map<std::string, float> url_rank;
...
void sim(){
	...
	std::unordered_map<std::string, float>::iterator url_rank_it;
	std::unordered_map<int, std::string>::iterator id_url_it;

	float numerador = 0, den1 = 0, den2 = 0, result;
	float beta = 0.6, tf_idf_rank;
	for(final_tf_idf_docs_it = final_tf_idf_docs.begin(); final_tf_idf_docs_it != final_tf_idf_docs.end(); ++final_tf_idf_docs_it){
		id_url_it = id_url.find(final_tf_idf_docs_it->first);
		url_rank_it = url_rank.find(id_url_it->second);
		numerador = 0;
		den1 = 0;
		den2 = 0;
		...
		result = (numerador / (sqrt(den1) * sqrt(den2)));
		tf_idf_rank = ((0.5)+(0.5)*tanh( (pow(10, 3.1) * (url_rank_it->second) )/2.0));
		//suaviza o pagerank utilizando de uma funcao sigmoide
		result = (-1)*(result*beta + tf_idf_rank*(1-beta));
...
void parser(){
    std::ifstream in("data/pagerank.jsonl", std::ios::in | std::ios::binary);
    if (!in) {
      std::cout << "Arquivo pagerank.jsonl não encontrado!\n";
      exit(EXIT_FAILURE);
    }
    std::string line;
    while (std::getline (in,line)){
        ...
        url_rank.insert({document["url"].GetString(), document["pagerank"].GetFloat()});
    }
}
